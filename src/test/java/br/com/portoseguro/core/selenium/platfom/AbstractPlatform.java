...
public abstract class AbstractPlatform implements Platform {
	...
	public WebDriver getRemoteWebDriver(String host, int port)   {		
		...
		if (trimmedHost.equals("localhost") ) { return getLocalWebDriver(); }
		try {
			
			RemoteWebDriver remoteWebDriver = new RemoteWebDriver(urlForRemoteHost(host, port), getDesiredCapabilities());
