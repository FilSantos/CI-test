...
import br.com.testfilipe.core.selenium.searchelements.SearchElementType;
...
public class CommandActionWeb extends CommandAction {
	...
	public void focus(WebElement webElement) {
		...
		if (!isHtmlUnit()) ((JavascriptExecutor) webDriver).executeScript(FOCUS_SCRIPT, webElement);
	...
	public void highlight(WebElement webElement) {
		webDriverWait.until(ExpectedConditions.visibilityOf(webElement));
		if (!isHtmlUnit())	((JavascriptExecutor)webDriver).executeScript(HIGHLIGTH_SCRIPT, webElement);
